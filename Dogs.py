...
def get_dog_image():
    try:
        ...
        mb.showerror("Error", f"Возникла ошибка при запросе к API {e}")
        return None
...
def show_image():
    ...
    if image_url:
        try:
            ...
            mb.showerror("Error", f"Возникла ошибка при загрузке изображения {e}")
    progress.stop()
...
progress = ttk.Progressbar(mode='determinate', length=300, orient=HORIZONTAL)
